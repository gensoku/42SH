...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   link_double_right_redir.c                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tsedigi <tsedigi@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/02/12 07:17:49 by tsedigi           #+#    #+#             */
/*   Updated: 2017/02/15 04:56:08 by tsedigi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "sh_func.h"

static	int		norm_linkerr_dra_(t_token *node, t_agreg *t, int opt, char *tr)
{
	if (!opt)
...
static	int		set_fd_dra(int val, int fd, t_agreg *t, t_env *e)
{
	if (val == 1)
		e->fd[0] = fd;
	else if (val == 2)
...
int				link_tok_right2(t_token *node, t_env *e)
{
	...
	if ((fd = open(trimed, O_RDWR | O_APPEND)) == -1)
		return (norm_linkerr_dra_(node, t, 0, trimed));
	if (set_fd_dra(t->left_val, fd, t, e) == -1)
	{
		close(fd);
		return (norm_linkerr_dra_(node, t, 1, trimed));
	}
	node->fd = fd;
